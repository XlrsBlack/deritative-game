...
Felis et imperdiet euismod, purus ipsum pretium metus, in lacinia nulla nisl eget sapien. Donec ut est in lectus consequat consequat. Etiam eget dui. Aliquam erat volutpat. Sed at lorem in nunc porta tristique.";
    ...
    const string TEXDRAW_TMP = "TEXDRAW_TMP";
    ...
    public static void SetTextMeshProIntegration()
    {
        if (IsDefined(TEXDRAW_TMP))
        {
            if (EditorUtility.DisplayDialog("Removing TEXDRAW_TMP",
                 "This will remove TMP integration for all platform. (Files will be keep). Please Rebuild TEXDraw font data after recompile.",
                 "Proceed", "Cancel"))
        ...
        else
        {
            ...
            respond = EditorUtility.DisplayDialogComplex("Add TEXDRAW_TMP integration for all platform",
                 "TEXDraw TMP Integration is not straightforward. This process will take a moment.\n" +
                 "For your safety, we can automate the whole process. What do you prefer?\n" +
                 "(Just press enter if you're confused)",
                 "Automatic", "Manual", "Abort");
            if (respond == 2)
                return;
            if (respond == 1)
            {
    ...
    private static void SetDefined(string defineName, bool enable, bool forAll)
    {
        ...
        {//if (EditorUserBuildSettings.build)
